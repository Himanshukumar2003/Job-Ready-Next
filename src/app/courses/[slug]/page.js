...
function CourseDetails({ params }) {
  ...
  if (!course) {
    return (
      <h2 className="text-center text-red-600 text-2xl">Course Not Found</h2>
    );
  }
  const [pound, decimal] = course.price.split(".");
          ...
          <div className="grid grid-cols-12 gap-x-0 md:gap-x-4">
            <div className="col-span-12 lg:col-span-6">
              <div className="flex flex-col justify-center h-full gap-2">
                <h2 className="-text--theme-sm md:-text--theme-2xl capitalize -font--theme-Extrabold  -text--theme-light">
                  {course.title}
                </h2>
                ...
                <div className="flex gap-4 mb-4">
                  <p className="mt-4  text-white text-lg font-semibold">
                    ...
                    <span className="price">
                      ...
                      {pound}<span>.</span><span className="price-small">{decimal}</span>
                    </span>
                  </p>
        ...
        <div className="section-padding">
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 mt-4">
            <div className="sm:col-span-12 flex flex-col ">
              <h1 className="-text--theme-text-primary -font--theme-Extrabold -text--theme-sm md:-text--theme-xl lg:-text--theme-lg capitalize leading-tight mb20">
                {course.title}
              </h1>
          ...
          <div className="">
            {course.targetAudience &&
              course.targetAudience.audienceList &&
              course.targetAudience.audienceList.length > 0 && (
                <div className="mt-4">
                  <h3 className="text-2xl font-medium mb-6">
                    Who is this course for?
                  </h3>
                  <p className="line-clamp-1 text-gray-900 text-lg mt-4">
                    {course.targetAudience.peraGraph}
                  </p>
                  <ul className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                    {course.targetAudience.audienceList.map(
                      (audience, index) => (
                        <li
                          key={index}
                        ...
                        >
                          <span className="text-xl text-[var(--theme-primary-one)] group-hover:text-white">
                            ✔
                          </span>
            ...
            {course.courseDetails && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 my-5">
                {course.courseDetails.map((item, index) => (
                  <div
                    key={index}
                    className="flex flex-col items-center text-center space-y-4 p-4 rounded-lg"
                    style={{ border: "2px solid var(--theme-primary-one)" }}
                  >
                    <div
                      className="p-3 bg-pink-50 font-bold rounded-full group-hover:bg-pink-100 transition-colors"
                      style={{
                        fontSize: "35px",
          ...
          <div className="mt-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {course.courseContent && (
                <>
                  {Object.values(course.courseContent).map(
                    (courseContent, index) => (
                      <div key={index} className="p-4 border rounded-lg">
                        <div className="flex items-center gap-2 mb-3">
                          {/* <AlertCircle className="h-5 w-5 text-red-500" /> */}
                          <h3 className="text-lg font-semibold">
                            {courseContent.heading}
                          </h3>
                        </div>
          ...
          <div>
            {course &&
              ...
              course.learningOutcomes.heading && (
                <>
            ...
            <div className="flex flex-col gap-2 mt-4">
              ...
              course.learningOutcomes.content.length > 0 ? (
